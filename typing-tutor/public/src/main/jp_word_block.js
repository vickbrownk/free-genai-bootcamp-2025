class JpWordBlock {
	...
	set(x_offset,y_offset,txt_kanji,txt_romaji){
		...
		const y_kanji = 0 + y_offset

		const x_romaji = block_offset + x_offset
		const y_romaji = y_offset - (this.word.size_kj/2)
		...
		this.romaji.setText(txt_romaji)
		this.romaji.x = x_romaji
		this.romaji.y = y_romaji
		this.romaji.visible = this.scene.showRomaji()
	}
	...
	highlight(user_input) {
		if (this.falling === false) {
			...
			this.match_per = 1
			return 
		} else if (user_input === false || user_input === undefined) {
			this.match_per = 0
			return
		}
	...
	// Test cases
	//console.log(sequentialMatch(["ka", "ku", "ke"], "kakuke")); // Output: 3
	//console.log(sequentialMatch(["ka", "ku", "ke"], "kakuk")); // Output: 2
	//console.log(sequentialMatch(["ka", "ku", "ke"], "kaku")); // Output: 2
	//console.log(sequentialMatch(["ka", "ku", "ke"], "kake")); // Output: 1
	//console.log(sequentialMatch(["ka", "ku", "ke"], "kukake")); // Output: 0
	...
	update(){
		...
		this.kanji.y = this.word.y;
		this.kanji_masked.y = this.word.y;
		this.romaji.y = this.word.y - (this.word.size_kj/2);
		this.update_mask()
	}
	...
	remove(){
		...
		this.kanji.y = 0
		this.kanji_masked.visible = false
		this.kanji_masked.x = 0
		this.kanji_masked.y = 0
		this.romaji.visible = false
		this.romaji.x = 0
		this.romaji.y = 0
		this.falling = false
